  ...
  export default {
    ...
    computed: {
      ...
      priceAtacado () {
        const price = getPrice(this.product)
        if (this.extraDiscount.value) {
          return getPriceWithDiscount(price * this.atacado, this.extraDiscount)
        }
        return price * this.atacado
      },
      ...
      installmentValueAtacado () {
        if (this.canShowPriceOptions && this.installmentsNumber >= 2) {
          if (!this.monthlyInterest) {
            return (this.price * this.atacado) / this.installmentsNumber
          } else {
            const interest = this.monthlyInterest / 100
            return (this.price * this.atacado) * interest /
              (1 - Math.pow(1 + interest, -this.installmentsNumber))
          }
        }
        return 0
